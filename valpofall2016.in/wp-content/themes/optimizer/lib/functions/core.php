...
function optimizer_search_form( $form ) {
    ...
    <div>
    <input placeholder="' . esc_attr( 'Search &hellip;', 'optimizer' ) . '" type="text" value="' . get_search_query() . '" name="s" id="s" />
    <input type="submit" id="searchsubmit" value="'. esc_attr( 'Search', 'optimizer' ) .'" />
    </div>
    </form>';
